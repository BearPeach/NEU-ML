...
if __name__ == '__main__':
    ...
    test_img_path = '../imgs/001268.jpg'
    test_xml_path = '../imgs/001268.xml'
    ...
    cv2.imwrite('detect_result.png', dst)  # 保存图片到当前目录
    print("检测结果已保存为 detect_result.png")
    #cv2.imshow('img', dst)
    #cv2.waitKey(0)


def detect_car(input_img_path, output_img_path):
    device = get_device()
    transform = get_transform()
    model = get_model(device=device)
    gs = selectivesearch.get_selective_search()
    ...
    dst = copy.deepcopy(img)
    test_xml_path = None  # 不需要xml
    # bndboxs = util.parse_xml(test_xml_path)  # 不画gt
    # 候选区域建议
    selectivesearch.config(gs, img, strategy='f')
    rects = selectivesearch.get_rects(gs)
    svm_thresh = 0.60
